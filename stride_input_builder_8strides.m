...
for n = 1:length(indx)
    ...
    for b = 1:numel(short_bouts)
        ...
        if num_strides < 8
            ...
            for j = 1:floor(num_strides/8)
            events.strideStart = short_bouts(b).strides.strideStart(1+((j-1)*8):8+((j-1)*8));
            events.strideEnd = short_bouts(b).strides.strideEnd(1+((j-1)*8):8+((j-1)*8));
            agg_str{c} = aggEightStrides_norm(events,short_bouts(b).thigh_acc,short_bouts(b).chest_acc);
            ABC = ones(size(agg_str{c},2),1) * abc;
            agg_str_abc{c} = [ABC'; agg_str{c}];
            sub_ind(c) = n;
            sub_name{c} = sub;
            fall_labels(c) = fall;
            timeSinceStart(c) = short_bouts(b).startTime;
            c = c + 1;
            end
        end
    end
