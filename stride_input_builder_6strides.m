...
for n = 1:length(indx)
    ...
    for b = 1:numel(short_bouts)
        ...
        if num_strides < 6
            ...
            for j = 1:floor(num_strides/6)
            events.strideStart = short_bouts(b).strides.strideStart(1+((j-1)*6):6+((j-1)*6));
            events.strideEnd = short_bouts(b).strides.strideEnd(1+((j-1)*6):6+((j-1)*6));
            agg_str{c} = aggSixStrides_norm(events,short_bouts(b).thigh_acc,short_bouts(b).chest_acc);
